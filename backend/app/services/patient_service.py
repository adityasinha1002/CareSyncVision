...
def get_db_session():
    """Return the active SQLAlchemy session bound to the current app context."""
    return db.session
    ...
    def _check_and_create_alerts(self, patient_id, risk_score):
        ...
        try:
            ...
            if risk_score < self.RISK_HIGH_THRESHOLD:
                unresolved_alerts = db.session.execute(
            ...
            if risk_score >= self.RISK_HIGH_THRESHOLD:
                existing_alert = db.session.execute(
                    db.select(Alert).where(
                        Alert.patient_id == patient_id,
                        Alert.alert_type == 'health',
                        Alert.resolved_at == None
